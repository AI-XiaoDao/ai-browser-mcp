...
3. 封面文字建议：**AI Browser MCP · 268 tools · Windows · 一句话采集/逆向/定位**

## About 描述（建议）

**英文（GitHub 默认）：**
```
Windows local MCP browser automation — 268 tools, any AI agent (Cursor/Claude/Cline). One sentence: scrape, reverse, locate. FBrowser CEF · MIT · x64 + win32
```

**中文（Discussions / 简介补充）：**
```
Windows 本地 MCP 浏览器自动化 — 268 工具，Cursor/Claude 一句话：采集、逆向 POST、CDP 定位。下载即用，MIT 开源。
```
...
gh repo edit AI-XiaoDao/ai-browser-mcp `
  --description "Windows local MCP browser automation — 268 tools, any AI agent (Cursor/Claude/Cline). One sentence: scrape, reverse, locate. FBrowser CEF · MIT · x64 + win32" `
...
| 文件 | 平台 |
|------|------|
| `AI-Browser-MCP-x64-v2.8.0.zip` | 64 位运行包 |
| `AI-Browser-MCP-win32-v2.8.0.zip` | 32 位运行包 |
| `AI-Browser-MCP-cpp-x64-v2.8.0.zip` | x64 C++ 对照 |
| `AI-Browser-MCP-cpp-win32-v2.8.0.zip` | win32 C++ 对照 |
