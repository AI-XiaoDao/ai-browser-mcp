...
2. 上传图片：推荐基于浏览器自动化演示截图制作 1280×640 封面
3. 封面文字建议：**AI Browser MCP · 255 tools · Windows · 一句话采集/逆向/定位**

## About 描述（建议）

**英文（GitHub 默认）：**
```
Windows local MCP browser automation — 255 tools, any AI agent (Cursor/Claude/Cline). One sentence: scrape, reverse, locate. FBrowser CEF · MIT · x64 + win32
```
...
gh repo edit AI-XiaoDao/ai-browser-mcp `
  --description "Windows local MCP browser automation — 255 tools, any AI agent (Cursor/Claude/Cline). One sentence: scrape, reverse, locate. FBrowser CEF · MIT · x64 + win32" `
...
| 文件 | 平台 |
|------|------|
| `AI-Browser-MCP-x64-v2.8.2.zip` | 64 位运行包 |
| `AI-Browser-MCP-win32-v2.8.2.zip` | 32 位运行包 |
| `AI-Browser-MCP-cpp-x64-v2.8.2.zip` | x64 C++ 对照 |
| `AI-Browser-MCP-cpp-win32-v2.8.2.zip` | win32 C++ 对照 |
...
宣传文案见仓库根目录 [OPEN_SOURCE.md](../OPEN_SOURCE.md) · 英文 [QUICKSTART_EN.md](../CEFbro/AI-Fbowser-Mcp/docs/QUICKSTART_EN.md)。

## SEO 检查清单

- [x] GitHub Topics（15 个，含 `web-scraping` `playwright-alternative` 等）
- [x] README 首段 + 对比表 + 常见搜索问题（`<details>`）
- [x] [QUICKSTART_EN.md](../CEFbro/AI-Fbowser-Mcp/docs/QUICKSTART_EN.md) 英文长尾词
