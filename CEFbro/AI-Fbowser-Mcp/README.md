...
> Windows 本地浏览器自动化 MCP 服务端 — 真实 FBrowser CEF 内核 · 334 个自动化工具 · 仅监听 `127.0.0.1:9222` · MIT 开源

---

## 这是什么？

**AI浏览器 MCP Server** 是一个 **Windows 本地浏览器自动化 MCP 服务端**。它运行真实的 **FBrowser CEF (Chromium Embedded Framework)** 浏览器内核，通过 **Model Context Protocol (MCP 模型上下文协议)** 向 AI 编程助手暴露 **334 个浏览器自动化工具**（实际数量以 `http://127.0.0.1:9222/tools/list` 返回为准）。

### 与 Playwright / Puppeteer 的差异

| 对比项 | Playwright / Puppeteer | AI 浏览器 MCP |
|----|----------------------|--------------|
| 环境 | 需安装 Node + 驱动 + 编写脚本 | 解压即用，双击运行 |
| AI 集成 | 需编写 Playwright 代码 | **334 个预封装 MCP 工具**，自然语言调用 |
| 浏览器内核 | 受 `navigator.webdriver` 等特征影响 | **CEF 真实浏览器内核** + 指纹伪装 |
| 逆向分析 | 手动 Hook + 浏览器 DevTools | 内置 **CDP 断点引擎** + 函数级 Hook |
| 网络位置 | 视部署而定 | **127.0.0.1 纯本地**，数据不出本机 |
| 批量执行 | 自行编排脚本 | `batch` 单次最多 200 条命令顺序执行 |

### 主要使用场景

- **Web Scraping 网页数据采集** — `scrape` 单次调用完成 导航→等待→提取
- **JS Reverse Engineering JS 逆向分析** — 函数 Hook / 调用栈追踪 / 字符串解密 / 验证闭环
- **CDP Debugger 断点调试** — `debugger_flow` 单次调用完成 断点→求值→resume
...
前往 **[GitHub Releases](https://github.com/AI-XiaoDao/ai-browser-mcp/releases)** 下载最新版：
...
解压 → 双击 `AI-Fbowser-Mcp.exe` → 自动启动 MCP 服务（关闭浏览器窗口=退出程序；保持运行请最小化控制台窗口；零基础用户可参考 [docs/快速使用指南.md](docs/快速使用指南.md)）  
...
| **自动化** | 3 | `wait` (8种条件), `scrape` (单次调用爬取), `extract` (links/images/tables) |
...
| **断点调试** | 15 | `debugger_flow` (组合流程), `debugger_auto` (循环), `debugger_enable/resume/step_*`, `debugger_set_breakpoint`, `debugger_evaluate`, `debugger_inspect`, `debugger_wait_paused` |
...
| `http://127.0.0.1:9222/cursor-config` | 动态 Cursor 配置 |
...
| [快速使用指南](docs/快速使用指南.md) | 新用户 — 下载、双击、配置 Cursor |
