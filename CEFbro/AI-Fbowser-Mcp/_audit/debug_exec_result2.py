# -*- coding: utf-8 -*-
r"""诊断2: mcp_result 用 request_id 轮询。
"""
import json
import os
import re
...
loop.kill_app()
loop.start_app()
call("browser_navigate", {"url": "https://example.com/?dbg156b=%d" % int(time.time())})
...
print('task_id=', tid)
for i in range(8):
    e2, t2, d2 = call("mcp_result", {"request_id": tid}, to=30)
    print('  poll#%d err=%s %.2fs %s' % (i, e2, d2, t2[:400]))
    time.sleep(1.0)
