# -*- coding: utf-8 -*-
r"""第163轮: radio 群ID(取分组ID)回读验收。

判据:
  ① 基线健康
  ② set 规格: radio(26503, 群9) → 成功
  ③ arm(首臂) → get: applied=1; verified≥2(类型回读+分组ID回读); apply_failed 空
  ④ 收尾健康
"""
import json
import os
...
call("browser_navigate", {"url": "https://example.com/?r163group=%d" % int(time.time())})
health('①execute_js 快')

e, t, d = call("browser_context_menu", {"action": "set",
                                        "items": "radio|自建单|26503|9|0|"}, to=60)
rec('②set 成功(spec_lines=1)', (not e) and payload(t).get("spec_lines") == 1, "%.2fs" % d)
...
p = payload(t)
rec('③applied=1', (p.get("last_applied_items") or 0) == 1, "applied=%s" % p.get("last_applied_items"))
rec('③verified≥2(类型回读+分组ID回读)', (p.get("verified_items") or 0) >= 2, "verified=%s" % p.get("verified_items"))
af = p.get("apply_failed") or ""
rec('③apply_failed 空(群ID回读通过)', af == "", af[:100])
