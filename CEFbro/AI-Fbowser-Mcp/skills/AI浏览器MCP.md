...
| 3.0.0 | **逆向套件重构 + 16 新工具**（现 265 工具）：旧 `reverse_*` 独立工具并入 `browser_reverse_*` 统一命名/action 通道；新增 **UX 6**（`browser_snapshot`/`browser_click_text`/`browser_get_forms`/`browser_highlight`/`browser_fill_form`/`browser_element_action`）、**滚动 2**（`browser_get_scroll`/`browser_scroll_by`）、**断点管理 2**（`browser_debugger_list_breakpoints`/`browser_debugger_clear_breakpoints`）、**逆向 6**（`browser_reverse_scan_crypto`/`browser_reverse_string_refs`/`browser_reverse_detect_obfuscator`/`browser_reverse_hook_logs`/`browser_reverse_hook_multi`/`browser_reverse_stack_trace`）；移除 `browser_create_tab`/`browser_task_runner_post`；`browser_event` 时间线模式 + 轻量事件监控默认开启 |
