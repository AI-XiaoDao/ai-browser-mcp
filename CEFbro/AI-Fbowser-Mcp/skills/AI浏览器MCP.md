---
name: AI浏览器MCP
description: AI浏览器 MCP 服务工具参考 - 255个浏览器自动化工具 (v2.8.2: AI交互6工具/browser_reverse_*套件/断点注册表/事件时间线/HAR导出/retry)
version: 2.8.2
...
---
# AI 浏览器 MCP 服务技能书 v2.8.2

## 概述
AI浏览器 MCP Server 基于火山 FBrowser CEF 框架，默认运行在 `ws://127.0.0.1:9222`。
**255 个 MCP 工具**（含 2 个 GUI 受限：`browser_create` / `browser_close`）。
...
> ⛔ **受限工具（不推荐使用）：** `browser_create` `browser_close` `browser_close_try` — 创建/关闭浏览器可能导致资源泄漏或会话中断，仅在明确需要时使用。
...
| `browser_get_source` | `max_chars`(可选,255KB) | 获取HTML源码 ⏳ |
| `browser_get_text` | `max_chars`(可选,255KB) | 获取页面文本 ⏳ |
...
| `browser_event` | `event_type`(空=最近事件时间线),`limit` | 查询事件 |
| `browser_network_body` | `request_id` | 响应体 🔒 ⏳ |
| `browser_snapshot` | `max_items`,`include_links`,`text_length` | AI页面快照: 交互元素清单(索引/文本/选择器) ✨ |
| `browser_click_text` | `text`,`selector?`,`index`,`exact` | AI点击: 按可见文本点击 ✨ |
| `browser_get_forms` | `form_index?` | AI表单枚举: 全部表单字段 ✨ |
| `browser_fill_form` | `fields`,`submit?` | AI一键填表: 批量填写+存在性检查 ✨ |
| `browser_highlight` | `selector`,`action`,`duration_ms` | AI高亮: 页面元素红色虚线高亮 ✨ |
| `browser_reverse_scan_crypto` | `script_index?` | 逆向定位: 加密算法特征扫描(MD5/AES/SHA/自定义base64表/RSA) 🔍 |
| `browser_reverse_string_refs` | `value`,`max_results?` | 逆向定位: 字符串引用位置(行号+上下文) 🔍 |
| `browser_reverse_detect_obfuscator` | `script_index?` | 逆向解密: 混淆器识别(obfuscator.io/JSVMP/JsJiami/sojson) 🔍 |
| `browser_reverse_hook_logs` | `log_key`,`action` | 逆向解密: Hook日志查询(eval/websocket/cookie) 🔍 |
...
# 2) 一键预备 + 注入 XHR/fetch Hook（见 scenarios/xhr_encrypt_scan.js）
...
**疑似加密字段启发式**（`xhr_encrypt_scan.js`）：
- 字段名：`sign`、`token`、`verify`、`stub` 等（视目标站点而定）
- 值特征：高熵 Base64/hex、超长随机串
- 同时分析 **URL query** 与 **body**（JSON / `URLSearchParams`）

| 脚本 | 用途 |
|------|------|
| `scenarios/xhr_encrypt_scan.js [url]` | 目标站点 XHR/fetch POST 加密字段扫描 |
...
```bash
# AI-Fbowser-Mcp.exe 已启动后
node full_test.js                 # 全量冒烟回归（含 AI交互/修复回归）
node regression_sync_wait.js      # sync-wait / workflow / batch 专项
```

测试覆盖全部 255 个 MCP 工具（完整工具清单用 `mcp_bridge.js --tools`）。

### 分项脚本（仓库内已提供）

| 脚本 | 用途 |
|------|------|
| `full_test.js` | HTTP 冒烟 + AI交互/修复回归（15 项） |
| `regression_sync_wait.js` | sync-wait / workflow / batch 专项（13 项） |
| `workflow_runner.js --server hello` | 工作流端到端 |

> 注：`tool_test_all.js`、`scenario_test.js`、`workflow_runner.js` 与 `scenarios/` 系列为内部测试工具，未包含在本开源仓库中；同等能力可直接用 MCP 工具验证（如 `browser_debugger_resume` 解除暂停、`browser_collect reverse_prepare` 预备扫描、`browser_restore_gui` 恢复界面）。
