...
version: 1.1
trigger: 场景测试|Hook测试|reverse_analyze|debugger_session|automation_flow|run_all_scenarios|js_http_hook|v8_hook|xhr_encrypt|POST加密|debugger_unfreeze
---
# AI 浏览器 — 场景与 Hook 测试技能书

## 何时阅读
- 需要 **XHR/fetch POST 参数**、判断哪些字段加密/签名时
- 用户反馈 persist 注入、network 日志、workflow 超时、debugger 卡死时

> ⚠️ 本文提到的 `scenarios/` 系列与 `run_all_tests.js` 等为内部测试工具，**未包含在本开源仓库中**；下列命令仅作能力参考。仓库内可用的验证脚本为 `full_test.js`、`regression_sync_wait.js`（项目根）；同等能力也可直接用 MCP 工具完成（见各节说明）。

## 快速命令（内部工具参考）

```bash
# 场景顺序包（内部脚本示例）
node CEFbro/AI-Fbowser-Mcp/scenarios/run_all_scenarios.js
node CEFbro/AI-Fbowser-Mcp/scenarios/run_all_scenarios.js --quick   # 跳过 reverse_analyze 外网

# 网络 / 逆向
node CEFbro/AI-Fbowser-Mcp/scenarios/reverse_analyze.js https://example.com/
node CEFbro/AI-Fbowser-Mcp/scenarios/xhr_encrypt_scan.js
node CEFbro/AI-Fbowser-Mcp/scenarios/xhr_encrypt_scan.js https://example.com/

# Hook 冒烟
node CEFbro/AI-Fbowser-Mcp/scenarios/v8_hook_test.js
node CEFbro/AI-Fbowser-Mcp/scenarios/js_http_hook_test.js

# 断点
node CEFbro/AI-Fbowser-Mcp/scenarios/debugger_session.js --flow --url URL
node CEFbro/AI-Fbowser-Mcp/scenarios/debugger_inspect_demo.js

# 卡死恢复
node CEFbro/AI-Fbowser-Mcp/scenarios/debugger_unfreeze.js
node CEFbro/AI-Fbowser-Mcp/scenarios/debugger_recover.js
```

## mcp_client.js 公共 API

场景脚本共用 `mcp_client.js`（仓库根目录，HTTP POST `http://127.0.0.1:9222/mcp`）：
...
| 加密字段识别 | — | ✅ `xhr_encrypt_scan.js` 启发式 |
...
### XHR POST 加密扫描（xhr_encrypt_scan.js）

- 默认打开 `https://example.com/`；`--wait N` 控制最长等待，`--min N` 够条数提前结束
...
- 标注疑似加密：`sign`、`token`、`verify`、`stub` 等字段名 + 高熵值
- 过滤目标站点无关域名；无则分析全部 POST
...
- 卡死恢复（MCP 工具路径）：`browser_debugger_resume`（resume×3）→ `browser_debugger_flow` disable → `browser_navigate` 欢迎页 → `browser_restore_gui`
- 恢复后可用 `browser_debugger_last_paused` 验证状态
...
单浏览器 + MCP 锁：任何测试脚本与场景流程必须**顺序**执行，勿与多个 MCP 客户端并发调用。

## 脚本一览（内部工具参考，未随开源仓库发布）
...
| `xhr_encrypt_scan.js` | — | POST 加密扫描；`--wait` `--min` `--scroll` `--skip-navigate` |
