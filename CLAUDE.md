...
MCP 服务运行在 `ws://127.0.0.1:9222`，提供 **268 个浏览器自动化工具**（3 个 GUI 受限：`browser_create` / `browser_close` / `browser_create_tab`）。v2.8.0 核心增强：44 逆向 CDP 工具 / 7 反检测 / workflow 条件变量 / resources+prompts 协议。
...
| [AI浏览器MCP.md](CEFbro/AI-Fbowser-Mcp/skills/AI浏览器MCP.md) | **268 个 MCP 工具完整参考** v2.8（sync-wait、workflow条件变量、44逆向CDP、7反检测、HAR导出、retry） |
...
6. **测试脚本**: `tests/batch_test.js` — 分类全量测试(237工具)
