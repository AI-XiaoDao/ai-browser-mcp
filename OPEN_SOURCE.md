...
✅ 成品下载：GitHub Releases（exe + CEF，约 157MB）— **VIP 功能全部免费开放**
✅ 自编译：须自行向 FBrowser 内核作者申请 VIP 授权码
...
| **Release VIP 全开** | GitHub 成品 zip：**截图/CDP/调试器/指纹/拦截等全部免费**，无需 `vip_code` |
| **自编译 VIP** | 自行编译 exe 须向 **FBrowser 内核作者**申请授权码 |
...
▸ 浏览器与自动化
  ...
  · 事件 Hook 系统 + VIP 资源拦截，支持逆向与改包场景
  · CDP WebSocket 原生暴露，可接 DevTools 生态；VIP 调试器断点/单步/flow
...
**逆向算法**（POST body 须 Hook / VIP 拦截，见 FAQ）
```
...
③ 定位  「断点跟到 sign 函数，给源码」       → debugger_*（Release 全开）
...
**Q：VIP 是什么？Release 要填授权码吗？**  
A：**GitHub Release 成品 zip** 已**免费开放全部 VIP 功能**，下载解压即用，**无需** `vip_code`。若**自行从火山源码编译** exe，须**自行向 FBrowser CEF 内核作者**申请 VIP 授权码并写入 `mcp_config.json`。工具文档中的 🔒 标记指内核能力分类，不代表 Release 用户另付费。

**Q：Release 和自编译 VIP 有何区别？**  
A：Release 由项目维护者编译发布，VIP 已内置可用；自编译者使用的是您本机火山 IDE 环境下的 FBrowser 库，须自行联系内核作者授权。
...
**Q：逆向和定位有什么区别？**  
A：**逆向**侧重找「哪个请求/字段被加密、body 长什么样」（Hook + 网络对比）；**定位**侧重找「加密逻辑在哪个 JS 函数」（CDP 断点 + 栈 + 源码；**Release 成品已开放**）。
...
- **CDP 调试器**：断点、单步、资源拦截、指纹 — **Release 成品全部开放**；自编译须内核作者授权  
...
| 找 sign 函数 | 手动断点跟栈 | 一句话 + `debugger_*`（Release 全开） |
...
✅ 217 工具 · 本机 9222 · MIT
✅ 采集数据 / 逆向 POST / 定位 sign — AI 自己跑 · **Release VIP 全开**
✅ Windows exe 下载即用
