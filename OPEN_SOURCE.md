...
✅ 成品下载：GitHub Releases — 255 工具全开放，解压即用
...
**AI-Browser-MCP** —— 基于火山视窗 + FBrowser CEF 的 **Windows 本地浏览器自动化 MCP 服务**。原生 API 优先（12+ 工具已去 JS 化），255 MCP 工具，解压即用。
...
| 亮点 | 说明 |
|------|------|
| **255 MCP 工具** | 导航 / 填表 / DOM / JS / 网络 / 工作流 / CDP 等，24 大类 |
...
| **下载即全功能** | GitHub 成品 zip：**截图/CDP/调试器/指纹/拦截等 255 工具全部可用** |
...
▸ 对 AI 用户
  · 255 个 browser_* 工具，Cursor / Claude 直连 MCP，自然语言操控浏览器
...
▸ 一句话场景（不用写脚本）
  ...
  · Agent 自动串联 255 工具，sync-wait 逐步等结果，你只管说目标
...
```
打开目标网站，扫描 XHR/fetch 的 POST，标出疑似加密字段和依据。
...
```
[Open Source] AI Browser MCP v2.8 — 255 tools for Cursor on Windows
...
```
Title: [Release] AI Browser MCP Server v2.8 — local Windows browser automation for Cursor (255 MCP tools)

I open-sourced a Windows-native MCP server that wraps a real FBrowser CEF browser with 255 browser_* tools.
...
255 个 browser_* 工具已封装，Agent 自动串联，MIT 开源。
...
| 255 工具参考 | `CEFbro/AI浏览器/skills/AI浏览器MCP.md` |
...
A：本项目的 MCP 服务跑在**真实 FBrowser CEF 窗口**里，工具通过 JSON-RPC 暴露给 AI；你不需要在 Agent 里写 Node 浏览器脚本，直接用自然语言驱动 255 个已封装工具。
...
**Q：真的不用写脚本吗？一句话就够？**  
A：终端用户配置好 Cursor + exe 后，**描述目标即可**（采集/逆向/填表等）。Agent 会自动选 MCP 工具并逐步执行。复杂固定流程可另存 `workflows/*.json`；POST 参数逆向流程见 `skills/AI浏览器MCP.md` 网络逆向章节。
...
**AI浏览器 MCP Server** 换了一条路：在 Windows 上跑一个带 FBrowser CEF 的真实浏览器，对外暴露 **Model Context Protocol (MCP)** 接口，把 **255 个**常用操作封装成 `browser_*` 工具。你在 **任意 AI 代理**（Cursor、Claude、自研 MCP 客户端）里说一句话，AI 直接调工具，不用从零写自动化脚本。
...
- **255 个 MCP 工具**，Cursor 里说一句话即可导航、填表、读 DOM、抓网络  
...
**标题建议**：【开源】AI浏览器 MCP Server v2.8 — 255 工具 · FBrowser CEF · Cursor 桥接
...
255 MCP 工具｜采集数据 · 逆向 POST · CDP 定位 sign｜FBrowser CEF｜MIT
