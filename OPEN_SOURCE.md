...
✅ 成品下载：GitHub Releases — 268 工具全开放，解压即用
  · x64 ~157MB · win32 ~136MB
✅ 本机 127.0.0.1:9222，默认不暴露外网

下载：https://github.com/AI-XiaoDao/ai-browser-mcp/releases
  · AI-Browser-MCP-x64-v2.8.0.zip / AI-Browser-MCP-win32-v2.8.0.zip — 运行包
...
**AI-Browser-MCP** —— 基于火山视窗 + FBrowser CEF 的 **Windows 本地浏览器自动化 MCP 服务**。原生 API 优先（12+ 工具已去 JS 化），268 MCP 工具，解压即用。
...
| 亮点 | 说明 |
|------|------|
| **268 MCP 工具** | 导航 / 填表 / DOM / JS / 网络 / 工作流 / CDP 等，24 大类 |
...
| **下载即全功能** | GitHub 成品 zip：**截图/CDP/调试器/指纹/拦截等 268 工具全部可用** |
...
▸ 对 AI 用户
  · 268 个 browser_* 工具，Cursor / Claude 直连 MCP，自然语言操控浏览器
...
▸ 一句话场景（不用写脚本）
  ...
  · Agent 自动串联 268 工具，sync-wait 逐步等结果，你只管说目标
...
```
Title: [Release] AI Browser MCP Server v2.8 — local Windows browser automation for Cursor (268 MCP tools)

I open-sourced a Windows-native MCP server that wraps a real FBrowser CEF browser with 268 browser_* tools.
...
268 个 browser_* 工具已封装，Agent 自动串联，MIT 开源。
...
| 文件 | 大小 | 用途 |
|------|------|------|
| [AI-Browser-MCP-x64-v2.8.0.zip](https://github.com/AI-XiaoDao/ai-browser-mcp/releases/download/v2.8.0/AI-Browser-MCP-x64-v2.8.0.zip) | ~157MB | **运行包**：exe + CEF + 脚本/文档 |
| [AI-Browser-MCP-cpp-x64-v2.8.0.zip](https://github.com/AI-XiaoDao/ai-browser-mcp/releases/download/v2.8.0/AI-Browser-MCP-cpp-x64-v2.8.0.zip) | ~0.3MB | **C++ 对照**（可选，与仓库 `generated-cpp/` 相同） |

```
1. 解压 AI-Browser-MCP-x64-v2.8.0.zip 到任意目录
2. 双击 AI浏览器.exe
...
| 268 工具参考 | `CEFbro/AI浏览器/skills/AI浏览器MCP.md` |
...
A：本项目的 MCP 服务跑在**真实 FBrowser CEF 窗口**里，工具通过 JSON-RPC 暴露给 AI；你不需要在 Agent 里写 Node 浏览器脚本，直接用自然语言驱动 268 个已封装工具。
...
| **v2.8.0 Release** | https://github.com/AI-XiaoDao/ai-browser-mcp/releases/tag/v2.8.0 |
| **客户使用手册** | 仓库 `CEFbro/AI浏览器/docs/客户使用手册.md` |
| **268 工具参考** | 仓库 `CEFbro/AI浏览器/skills/AI浏览器MCP.md` |
...
*AI浏览器 MCP Server v2.8.0 · MIT License · 内核 FBrowser CEF · 端口 9222*
...
**AI浏览器 MCP Server** 换了一条路：在 Windows 上跑一个带 FBrowser CEF 的真实浏览器，对外暴露 **Model Context Protocol (MCP)** 接口，把 **268 个**常用操作封装成 `browser_*` 工具。你在 **任意 AI 代理**（Cursor、Claude、自研 MCP 客户端）里说一句话，AI 直接调工具，不用从零写自动化脚本。
...
- **268 个 MCP 工具**，Cursor 里说一句话即可导航、填表、读 DOM、抓网络  
...
1. 下载 Release：https://github.com/AI-XiaoDao/ai-browser-mcp/releases/tag/v2.8.0  
   解压 **AI-Browser-MCP-x64-v2.8.0.zip**，运行 **AI浏览器.exe**  
...
**标题建议**：【开源】AI浏览器 MCP Server v2.8 — 268 工具 · FBrowser CEF · Cursor 桥接
...
【项目】AI Browser MCP Server v2.8.0
【仓库】https://github.com/AI-XiaoDao/ai-browser-mcp
【成品】https://github.com/AI-XiaoDao/ai-browser-mcp/releases/tag/v2.8.0
...
268 MCP 工具｜采集数据 · 逆向 POST · CDP 定位 sign｜FBrowser CEF｜MIT
...
```
【开源】AI浏览器 MCP v2.8 — Cursor 一句话操控真实浏览器
