...
255 MCP tools for **any AI agent** (Cursor, Claude, Cline, custom MCP clients): navigate, fill forms, read DOM, network, workflows, CDP debugger.
...
✅ Binary: GitHub Releases — **x64 ~157MB / win32 ~136MB**, exe + CEF — **all 255 tools, download and go**
...
**AI Browser MCP Server** is a **Windows-native local MCP service** built with Volcano IDE + FBrowser CEF. **Any MCP-compatible AI agent** — Cursor, Claude Desktop, Cline, OpenCode, or your own app — can drive a real browser with **255** `browser_*` tools via HTTP/WebSocket or `mcp_bridge.js`.
...
| **255 MCP tools** | Navigate, fill forms, DOM, JS, network, workflows, CDP — 24 categories |
...
| **Download = full power** | Release zip includes screenshot, CDP, debugger, fingerprint, intercept — all 255 tools |
...
**Prompt:** “Open a target site, scan XHR/fetch POST and flag likely encrypted fields.”
...
5. Optional: `xhr_encrypt_scan.js` for reference
...
| Pre-built tools | **255** | roll your own | vendor-dependent |
...
Title: [Release] AI Browser MCP Server v2.8 — local Windows browser automation for any MCP AI agent (255 tools)



Windows-native MCP server wrapping FBrowser CEF with 255 browser_* tools.
...
| Docs & 255-tool reference | `docs/`, `skills/AI浏览器MCP.md` |
...
**AI Browser MCP Server** bridges the gap between AI assistants and a real Windows browser. Instead of asking Cursor to write Playwright scripts from scratch, you run a local exe that exposes **255** ready-made MCP tools.
