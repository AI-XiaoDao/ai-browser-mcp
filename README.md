...
本仓库开源的是 **火山视窗 `.wsv` 源码**及文档/脚本，**不包含** FBrowser CEF 闭源库与编译生成的 C++ 文件。
...
**哪个才是 C++ 源码？** → **`project/`** 目录。

火山将 `src/*.wsv` 翻译为 MSVC 可编译的 C++，例如 `vpkg_main.cpp`、`vpkg_MCP_Server.cpp` 及大量 `vcls_*.h`。文件头标注 `Generated by Volcano Development Platform`。这些文件**每次编译可再生**，故不纳入 Git；审计与二次开发请以 **`src/*.wsv`** 为准。
