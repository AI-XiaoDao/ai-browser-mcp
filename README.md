...
### Cursor / Claude 一句话操控 Windows 真实浏览器

**217 个 MCP 工具** · **FBrowser CEF 真实窗口** · **Release 成品 VIP 全开** · **本机 `127.0.0.1:9222`** · **MIT 开源**

*Local Windows MCP browser automation — no Playwright scripts required*
...
在 Windows 上运行 **AI浏览器.exe**，本机暴露 **Model Context Protocol (MCP)** 服务。Cursor / Claude 通过 `mcp_bridge.js` 调用 **217 个** `browser_*` 工具——**说一句话，Agent 自动串联执行**，无需手写 Playwright 或手动点 DevTools。
...
3. **配置 Cursor**（[`mcp_bridge.js`](CEFbro/AI浏览器/mcp_bridge.js) + 根目录 [`.mcp.json`](.mcp.json)）→ 对 AI 说：**「滚动列表，把标题价格采成 JSON」**

<details>
<summary><b>Cursor 配置 JSON（点击展开）</b></summary>
...
| 优点 | 说明 |
|------|------|
| **自然语言驱动** | 在 Cursor / Claude 里说需求，AI 自动选工具，无需从零写 Playwright/Puppeteer 脚本 |
...
| **欢迎页控制台** | `http://127.0.0.1:9222/` 查健康状态、浏览文档、复制 Cursor 配置 |
| **一句话自动执行** | 说需求即可：Agent 自选工具链（导航→Hook→读网络→断点→导出），全程 MCP 驱动 |

### 典型场景 · 一句话自动执行 {#一句话能干啥}

不用写脚本、不用记 217 个工具名。在 Cursor 里用自然语言描述目标，AI 会按场景自动组合 `browser_*` / `workflow_*` / VIP 调试器，并在每步 **sync-wait** 等结果后再决定下一步。
...
|------|---------------------------|--------------|
| 采商品列表 | 写选择器 + 滚动循环 + 解析 + 存文件 | Cursor：**「滚动采标题价格 JSON」** → Agent 自动调 MCP |
...
| 优点 | 说明 |
|------|------|
| **标准 MCP** | 实现 Model Context Protocol，兼容 Cursor、Claude Desktop 等 MCP 客户端 |
| **双通道 JSON-RPC** | **WebSocket** 长连接 + **HTTP POST** `/mcp`，脚本与 IDE 均可接入 |
| **Cursor 一键桥接** | `mcp_bridge.js`：stdio ↔ HTTP，仓库自带 `.mcp.json` 示例 |
...
|------|:------------:|:-------------------------:|:----------------:|
| AI 接入 | MCP 原生，Cursor 直连 | 需自己写 MCP 或脚本层 | 通常另购 API |
...
**Release 成品**由项目维护者编译发布，已内置可用 VIP 能力，方便终端用户与 Cursor Agent **开箱即用**（含采集、逆向 Hook、CDP 定位 sign 等全链路）。
