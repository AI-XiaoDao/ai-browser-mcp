...
- ✅ **C++ 源码** → 本 Release 的 `cpp` zip，或 Git 仓库 `CEFbro/AI浏览器/generated-cpp/release-x64/`
- ✅ **权威源码** → Git 仓库 `CEFbro/AI浏览器/src/*.wsv`（火山视窗）
- ❌ **`linker/out/`** → 仅为 `.obj`/`.pch` **编译中间产物**，**不是源码**；本 Release **不包含** 该目录
